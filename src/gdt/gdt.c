...
//Encodes the structured gdt entry to the correct input of the gdt for the processor
...
void gdt_structured_to_gdt(struct gdt* gdt, struct gdt_structured* structured, uint32_t total_entries)
{
    ...
    {
        encode_gdt_entry((uint8_t*) &gdt[i], structured[i]); //Parse them to proper gdt entries
    }
}